...
![IFGR method overview](figure1.png)

> **그림 1.** IFGR 전체 구조. SAFREE로 이미지를 1차 생성(Pass 1)한 뒤, 평가용 NudeNet과 독립적인 CLIP zero-shot NSFW 게이트로 결과를 스크리닝하고, unsafe로 판정된 이미지만 같은 seed에서 cross-attention 억제로 재생성(Pass 2)한다. benign 이미지는 게이트를 통과해 그대로 반환되어 품질이 보존된다.
